...
from opensora.models.diffusion.opensora.modules import BasicTransformerBlock, _UnpatchifyOutTime
    ...
    def forward(self, x, emb=None):
        ...
        if emb is not None:
            ...
            if emb.shape[0] > out.shape[0]:
                emb = emb[:out.shape[0]]  #TODO: study always 1st half?
...
class AggregationNetwork(nn.Module):
    def __init__(
        ...
        # self.bottleneck_layers = self.bottleneck_layers.to(dev)
        mixing_weights = torch.ones(len(self.bottleneck_layers) * len(save_timestep))
        self.mixing_weights = nn.Parameter(mixing_weights)  # .to(dev)
        ...
        if self.use_output_head:
            ...
            # self.output_head = self.output_head.to(dev)
        else:
...
class AggTsfm(nn.Module):
    def __init__(self, bas_tsfm_blk_cmd, num_feats, norm_type='ada_norm_single',
                 inner_dim=2304, patch_size_t=4, patch_size=2, out_channels=4, uprate=[0, 0, 0]):
        super(AggTsfm, self).__init__()
        self.blks = nn.ModuleList(
            [eval(bas_tsfm_blk_cmd.format(
                skips_in=num_feats * inner_dim))]
            + [eval(bas_tsfm_blk_cmd.format(skips_in=0)) for _ in range(4 - 1)])
        self.unpat_out_time = _UnpatchifyOutTime(
            norm_type=norm_type, inner_dim=inner_dim, patch_size_t=patch_size_t,
            patch_size=patch_size, out_channels=out_channels, uprate=uprate)
    ...
    def forward(self,
        ...
        for bidx, blk in enumerate(self.blks):
            args = [hidden_states_vid,
                    skips if not bidx else None,
                    None,  # latent_poses
                    attention_mask_vid,
                    encoder_hidden_states_vid,
                    encoder_attention_mask_vid,
            ...
            if self.training and gradient_checkpointing:
                ...
                hidden_states_vid = blk(*args)
        output_vid = self.unpat_out_time(
